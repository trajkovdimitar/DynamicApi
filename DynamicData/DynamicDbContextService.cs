...
public class DynamicDbContextService
{
    ...
    private static string GetProjectDirectory()
    {
        ...
        return Path.GetFullPath(Path.Combine(directory, "..", "..", "..", "..", "DynamicData"));
    }
    ...
    public async Task RegenerateAndMigrateAsync()
    {
        ...
        try
        {
            RunDotnetCommand($"ef migrations add AutoMigration_{DateTime.UtcNow:yyyyMMddHHmmss} --context DynamicDbContext --namespace DynamicData.Migrations --output-dir Migrations", ProjectDir);
        }
        catch (Exception ex)
        {
        ...
        _dynamicDbContextType = _dynamicAssembly.GetType("DynamicData.DynamicDbContext")
...
{{
    ...
    private string GenerateDbContextCode(List<ModelDefinition> models, string @namespace = "DynamicData")
    {
        var sb = new StringBuilder();
    ...
    public Type GetModelType(string modelName) =>
        _dynamicAssembly.GetType($"DynamicData.{modelName}")
